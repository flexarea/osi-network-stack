...
int main(int argc, char *argv[]){
	...
	struct table_r *routing_table = (struct table_r *) malloc(TABLE_LENGTH*sizeof(struct table_r));
...
void handle_packet(ssize_t len, struct frame_fields *frame_f, uint8_t *or_frame, struct arp_cache *arp_cache_){
	...
	if(lg_pfx_idx != -1){
		...
		for(int i=0; i<CACHE_LENGTH; i++){
			if((memcmp(arp_cache_[i].ip_addr, routing_table[lg_pfx_idx].gateway, 4)) == 0){
...
void encapsulation(struct frame_fields *frame_, uint8_t *arp_cache_, int arp_idx_, int lg_pfx_idx_, struct ip_header packet_, ssize_t len){
	/*encapsulation logic*/
	memcpy(frame_->src_addr, ether_addr, 6);
	memcpy(frame_->dest_addr, arp_cache_[arp_idx_].mac_addr, 6);
	memcpy(packet_->src_addr, interface_ip, 4);
	memcpy(packet_->dest_addr, routing_table[lg_pfx_idx_].dest, 4);
	packet_->ttl--;
	//recalculate checksum
	packet_->checksum = ip_checksum(packet_);	
	//recreate frame
	memcpy(frame_, packet_+14, len - 14);
}
